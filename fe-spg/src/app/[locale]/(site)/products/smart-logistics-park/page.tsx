...
import {
  getSmartLogisticsHandlingContent,
  smartLogisticsOverviewImage,
} from "@/content/products/smart-logistics-handling";
import { defaultLocale, isAppLocale } from "@/i18n/routing";
import { createLocalizedMetadata } from "@/lib/seo";

type PageProps = { params: Promise<{ locale: string }> };

const pageHref = "/products/smart-logistics-park";

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  const content = getSmartLogisticsHandlingContent(activeLocale);
...
export default async function SmartLogisticsParkPage({ params }: PageProps) {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  ...
  return (
    <>
      ...
      <ProductSystemDetailSection
        ...
        overviewImage={smartLogisticsOverviewImage}
      />
    </>
  );
}
