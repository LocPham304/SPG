...
import {
  breakbulkOverviewImage,
  getBreakbulkHandlingContent,
} from "@/content/products/breakbulk-handling";
import { defaultLocale, isAppLocale } from "@/i18n/routing";
import { createLocalizedMetadata } from "@/lib/seo";

type PageProps = { params: Promise<{ locale: string }> };

const pageHref = "/products/breakbulk-handling-systems";

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  const content = getBreakbulkHandlingContent(activeLocale);
...
export default async function BreakbulkHandlingSystemsPage({ params }: PageProps) {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  ...
  return (
    <>
      ...
      <ProductSystemDetailSection
        ...
        overviewImage={breakbulkOverviewImage}
      />
    </>
  );
}
