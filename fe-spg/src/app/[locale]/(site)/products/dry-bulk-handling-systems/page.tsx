...
import {
  dryBulkOverviewImage,
  getDryBulkHandlingContent,
} from "@/content/products/dry-bulk-handling";
import { defaultLocale, isAppLocale } from "@/i18n/routing";
import { createLocalizedMetadata } from "@/lib/seo";

type PageProps = { params: Promise<{ locale: string }> };

const pageHref = "/products/dry-bulk-handling-systems";

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  const content = getDryBulkHandlingContent(activeLocale);
...
export default async function DryBulkHandlingSystemsPage({ params }: PageProps) {
  const { locale } = await params;
  const activeLocale = isAppLocale(locale) ? locale : defaultLocale;
  ...
  return (
    <>
      ...
      <ProductSystemDetailSection
        ...
        overviewImage={dryBulkOverviewImage}
      />
    </>
  );
}
